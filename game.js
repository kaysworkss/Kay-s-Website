...
async function handleShopPaymentInit(req, res, supabase) {
  ...
  try {
    ...
    if (provider === 'paystack') {
      ...
      const paystackCurrency = checkout.method === 'ship' && checkout.zone !== 'NG-other' ? 'USD' : 'NGN';
      const paystackAmount = paystackCurrency === 'USD'
        ? Math.round(checkout.totalUsd * 100)
        : checkout.totalNgn * 100;
      const r = await fetch('https://api.paystack.co/transaction/initialize', {
        ...
        body: JSON.stringify({
          ...
          metadata: { name, phone, quote, display_total_ngn: checkout.totalNgn, display_total_usd: checkout.totalUsd },
        }),
      });
